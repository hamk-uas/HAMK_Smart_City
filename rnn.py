...
class RNN:
    ...
    def __init__(self, quant=None, seq=4, fut=0, parameters=None):
        ...
        self.x_parameters = parameters
        self.y_parameters = quant
        self.used_parameters = parameters + quant
    ...
    def preprocess(self, raw_data, split=0.2):
        ...
        df_train = raw_data[:int(len(raw_data)*(1-split))].copy()
        df_val = raw_data[int(len(raw_data)*(1-split)):].copy()
    ...
    def preprocess_output_date(self, raw_data, split=0.2):
        '''
        Function for preprocessing downsampled data for sequence modeling, output contains date to plot
        Inputs: Downsampled data frame with desired parameters defined in class attribute list in headers
        Output: Training input data, training target data, testing input data, testing target data, sklearn scaler object for inverse transformations
        '''
        raw_data.iloc[:, 0] = pd.to_datetime(raw_data.iloc[:, 0], format='%Y-%m-%d %H:%M:%S%z')
        ...
        self.parameters.extend(self.quant)
        # Split the data to training and testing sets
        df_train_raw = raw_data[:int(len(raw_data) * (1 - split))].copy()
        df_val_raw = raw_data[int(len(raw_data) * (1 - split)):].copy()
        raw_data = raw_data[self.parameters].copy()
        df_train = raw_data[:int(len(raw_data) * (1 - split))].copy()
        df_val = raw_data[int(len(raw_data) * (1 - split)):].copy()
        ...
        df_train_date = pd.DataFrame(df_train_raw['Time'].values)
        df_val_date = pd.DataFrame(df_val_raw['Time'].values)
        # print(df_val['Time'])
        ...
        for count, row in enumerate(pd.DataFrame(df_train).values):
            ...
            if (len(
                    prev_days_train) == self.seq):  # This checks that our sequences are of the correct length and target value is at full hour
                if (any(pd.isna(pd.DataFrame(df_train).values[count - 1][
                                -l_quant:]))):  # Test for 30 min data interval because of energy data gaps
                    continue
                try:
                    sequences_train.append(
                        [np.array(prev_days_train), pd.DataFrame(df_train).values[count + 1][-l_quant:]])
                except IndexError:
                    break
        ...
        for count, row in enumerate(pd.DataFrame(df_val).values):
            ...
            if (len(
                    prev_days_val) == self.seq):  # This checks that our sequences are of the correct length and target value is at full hour
                if (any(pd.isna(pd.DataFrame(df_val).values[count - 1][
                                -l_quant:]))):  # Test for 30 min data interval because of energy data gaps
                    continue
                try:
        ...
        return X_train, y_train, X_val, y_val,df_train_date,df_val_date



#     def preprocess_date(self,raw_data,on_date):
#         '''
#             preprocess downsampled data for sequence modeling which could split the training data and test data according to the date.
#             added by Wangkang
#             2022.05.30
...
#         idx = raw_data[raw_data.iloc[:,0]==on_date].index.values[0]
#         split = (len(raw_data)-idx-1)/len(raw_data)
#         print(split)
...
#     def preprocess_combined(self,raw_data,on_date,split=0.2): 
#         """
#         preprocess downsampled data for sequence modeling then change the test data from the beginning date of control.
#         added by Wangkang
#         2022.06.02
...
#         #calculate split rate
#         idx = raw_data[raw_data.iloc[:,0]==on_date].index.values[0]
#         split_date = (len(raw_data)-idx-1)/len(raw_data)
...
#         raw_data = raw_data[self.parameters].copy()
#         df_train = raw_data[:int(len(raw_data)*(1-split))].copy()
#         df_val = raw_data[int(len(raw_data)*(1-split_date)):].copy()
