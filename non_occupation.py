...
Energy optimization to reduce the inside temperature when the building is not occupied(16.00-8.00).
"""
...
def evening_cut(raw_data, fl,setting_temp = 20, next_morning=8, start_hour=18, pct=0.3,):
    """
    calculate end hour of cutting meanwhile the inside temperature is above setting one before next worktime.
    """

    ending = range(start_hour + 1, 24+next_morning+ 1)
    ...
    for ending_hour in ending:
        ...
        for i in range(len(X_test)):
            ...
            if X_test.loc[i, 'hours'] in range(next_morning,start_hour):
                ...
                if y_pred[0] >= setting_temp:
                    ...
                    end_output = ending_hour % 24-1
                    if ending_hour == 24:
                        end_output = 23
                    print("The longest {:1f}% cutting period from {} is to {} with {} hours prediction horizon".format(
                        (1-pct)*100, start_hour, end_output , f))
                    return end_output
            else:
                continue

        print('All temperatures are over {} ℃ from {} to {} with {} hours prediction horizon'.format(
                setting_temp,start_hour, ending_hour % 24, f))
    print("{}%".format((1-pct)*100))
    return next_morning

def plot_cut(raw_data,f,setting_temp,next_morning,start_hour,):
    """
    ...
    for j in np.arange(0,1,0.01):
        ending_hour = evening_cut(raw_data,f,setting_temp,next_morning,start_hour, j)
...
next_morning = 8
# plot
plot_cut(raw_data,f,setting_temp,next_morning,start_hour,)
